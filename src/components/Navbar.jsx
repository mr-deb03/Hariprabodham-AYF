...
// Browse links live in one row; Karyakarta (portal login) is pulled out as a
// distinct CTA button so it doesn't read as a crammed extra link.
const navLinks = [
  { label: "Home", to: "/" },
...
const cta = { label: "Karyakarta", to: "/portal" };
...
export default function Navbar() {
  ...
  // Big logo over the hero; on scroll the bar collapses to a compact layout.
  const rowHeight = scrolled ? "h-16" : "h-20 md:h-28";
  const logoSize = scrolled ? "h-11" : "h-16 md:h-24";
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50">
      ...
      >
        <div
          className={`mx-auto flex max-w-7xl items-center justify-between px-6 md:px-10 ${rowHeight}`}
        >
          {/* LOGO — left */}
          ...
          {/* DESKTOP NAV + CTA — right (lg and up) */}
          <div className="hidden items-center gap-7 lg:flex">
            <nav
              className={`flex items-center gap-7 text-[15px] font-semibold tracking-wide ${textColor}`}
            >
              {navLinks.map((link) => (
                <Link
                  key={link.label}
            ...
            <Link
              to={cta.to}
              className="rounded-full border border-gold/40 bg-maroon px-5 py-2 text-[15px] font-semibold tracking-wide text-ivory shadow-soft transition-all duration-200 hover:-translate-y-0.5 hover:bg-maroonDark hover:shadow-gold"
            >
              {cta.label}
            </Link>
          </div>
